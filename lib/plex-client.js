...
class PlexClient {
  constructor(serverUrl, token, options = {}) {
    // Debug logging for constructor parameters
    logger.plexDebug('PlexClient constructor called', {
      serverUrl,
      serverUrlType: typeof serverUrl,
      serverUrlConstructor: serverUrl?.constructor?.name,
      token: typeof token === 'string' ? token.substring(0, 8) + '...' : token,
      tokenType: typeof token,
      tokenConstructor: token?.constructor?.name
    });
    
    // Ensure serverUrl is a string and clean it up
    if (typeof serverUrl !== 'string') {
      logger.plexError('PlexClient constructor received non-string serverUrl, attempting conversion', {
        serverUrl,
        serverUrlType: typeof serverUrl,
        serverUrlConstructor: serverUrl?.constructor?.name
      });
      
      // Try to convert to string as a workaround
      try {
        serverUrl = String(serverUrl);
        logger.plexDebug('Successfully converted serverUrl to string', { convertedUrl: serverUrl });
      } catch (conversionError) {
        const error = new Error(`Server URL must be a string, got ${typeof serverUrl} (${serverUrl?.constructor?.name}): ${serverUrl}`);
        logger.plexError('PlexClient constructor failed', error);
        throw error;
      }
    }
