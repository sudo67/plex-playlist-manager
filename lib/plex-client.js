...
class PlexClient {
  ...
  async getLibraryContent(libraryId, limit = 10) {
    try {
  ...
  async searchLibrary(libraryId, query) {
    try {
      // Get content from the library
      const allContent = await this.getLibraryContent(libraryId, 100);
      
      // Filter content by title containing the query (case-insensitive)
  ...
  async searchMedia(query, type = null) {
    try {
      ...
      for (const library of libraries) {
        try {
          const results = await this.searchLibrary(library.key, query);
          // Add library info to each result
          const resultsWithLibrary = results.map(result => ({
            ...result,
            libraryTitle: library.title,
            libraryType: library.type
          }));
          allResults = allResults.concat(resultsWithLibrary);
        } catch (error) {
          // Continue searching other libraries if one fails
          logger.plexError('Library search failed', { library: library.title, error: error.message });
        }
      }
