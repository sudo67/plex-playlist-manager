...
class PlaylistManager {
  ...
  async getPlaylistDetails(playlistId) {
    return await this.showPlaylistDetails(playlistId);
  }

  async addTrackToPlaylist(playlistId, trackId) {
    const uri = `server://localhost/com.plexapp.plugins.library/library/metadata/${trackId}`;
    await this.client.addToPlaylist(playlistId, uri);
  }

  async removeTrackFromPlaylist(playlistId, trackId) {
    await this.client.removeFromPlaylist(playlistId, trackId);
  }
}
